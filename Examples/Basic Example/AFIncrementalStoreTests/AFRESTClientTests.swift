...
class AFRESTClientTests: XCTestCase {
    ...
    func test_ShoulBeCreateARequestForInsertedObject() {
        ...
        XCTAssertTrue((httpHeader?["Content-Type"]?.contains("application/x-www-form-urlencoded"))!, "should contains application/x-www-form-urlencoded")
        ...
        instanceRestClient.requestSerializer = AFJSONRequestSerializer(writingOptions: .prettyPrinted)
        ...
        XCTAssertTrue(httpBodyString == "{\n  \"artistDescription\" : \"test\",\n  \"name\" : \"test\",\n  \"birthDate\" : \"0000-12-30 00:00:00 +0000\"\n}", "should be equals to {\"artistDescription\":\"test\",\"name\":\"test\",\"birthDate\":\"0000-12-30 00:00:00 +0000\"}")

        httpHeader = result?.allHTTPHeaderFields
        XCTAssertNotNil(httpHeader, "should not be nil")
        XCTAssertTrue((httpHeader?["Content-Type"]?.contains("application/json"))!, "should contains application/json")
        XCTAssertNotNil(httpHeader?["User-Agent"], "should not be nil")
        XCTAssertNotNil(httpHeader?["Accept-Language"], "should not be nil")


        //encoding AFPropertyListParameterEncoding
        instanceRestClient.requestSerializer = AFPropertyListRequestSerializer(format: .xml, writeOptions: 0)
        result = instanceRestClient.request(forInsertedObject: artist)
        ...
        httpHeader = result?.allHTTPHeaderFields
        XCTAssertNotNil(httpHeader, "should not be nil")
        XCTAssertTrue((httpHeader?["Content-Type"]?.contains("application/x-plist"))!, "should contains application/x-plist")
        XCTAssertNotNil(httpHeader?["User-Agent"], "should not be nil")
        XCTAssertNotNil(httpHeader?["Accept-Language"], "should not be nil")
    ...
    func test_ShoulBeCreateARequestForUpdateObject() {
        ...
        httpBodyString = String(bytes: result!.httpBody!, encoding: .utf8)
        XCTAssertTrue(httpBodyString == "{\n  \"birthDate\" : \"4001-01-01 00:00:00 +0000\"\n}", "should be equals to {\"birthDate\":\"4001-01-01 00:00:00 +0000\"}")
